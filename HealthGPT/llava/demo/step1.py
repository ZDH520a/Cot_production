...
class DeepSeekBatchProcessor:
    ...
    def batch_process(self, message_list: List[List[Dict]], batch_size: int = 5, 
        ...
        all_results = []
        # Initialize a list to hold results with their original indices, initially None
        # This ensures that even if some requests fail, we can put the results back in order.
        final_ordered_results = [None] * len(message_list) 

        # Keep track of messages that need to be re-processed
        ...
        while messages_to_process:
            ...
            current_round_messages = messages_to_process # Messages for this round
            messages_to_process = [] # Reset for next round's failures
            ...
            for batch_idx_start in range(0, total_messages_in_round, batch_size):
                ...
                batch_messages_for_request = [item[1] for item in batch_data] # Extract messages only
                batch_original_indices = [item[0] for item in batch_data] # Extract original indices
                ...
                temp_batch_results = [None] * len(batch_data) # Store results for this batch temporarily
                with ThreadPoolExecutor(max_workers=max_workers) as executor:
                    ...
                    for future in as_completed(future_to_local_index):
                        ...
                        result["original_message_index"] = original_idx # Store global original index
                        temp_batch_results[local_idx] = result # Store in local ordered list
            ...
            if messages_to_process:
                ...
                time.sleep(5) # Longer pause before starting a new retry round

        print("\n所有轮次处理完成。")
        # Filter out any None values that might remain if all retries failed for some items
...
def main():
    ...
    parser.add_argument("--question-file", type=str, default="/root/autodl-tmp/HealthGPT/ZDH/train_0217/train_0217.json")
    parser.add_argument("--output-file", type=str, default="/root/autodl-tmp/HealthGPT/ZDH/train_0217/step1_results.json")
    parser.add_argument("--api-key", type=str, default="sk-3d990190200d42f2b500fe6d8117aa76")
    parser.add_argument("--batch-size", type=int, default=10)
    parser.add_argument("--max-workers", type=int, default=10)
    parser.add_argument("--num-samples", type=int, default=3000, 
                        help="要处理的样本数量。如果为None，则处理所有样本。")
    ...
    print("正在整理结果...")
    # Create a list of processed_data, ensuring it's ordered by original_index
    # First, create a temporary dictionary for quick lookup by original_index
    results_map = {res["original_message_index"]: res for res in results}
    
    processed_data = []
    # Iterate through the original questions list to maintain order and fill in results
    for idx, line in tqdm(enumerate(questions), desc="整理结果", total=len(questions)):
        result = results_map.get(idx)
        if result: # If a result exists for this original index
            processed_item = {
                "original_index": idx,
                "image": line["image"], 
                "question": line["question"],
                "answer": line["answer"],
                "img_path": os.path.join("/root/autodl-tmp/HealthGPT/playground/data_sft/img/train", line["image"] + ".jpg"),
                ...
                "step1_attempts": result.get("attempts", 1) # Record attempts
            }
