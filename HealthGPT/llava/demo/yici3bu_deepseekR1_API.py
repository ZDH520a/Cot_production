...
def infer():
    ...
    parser.add_argument("--question-file", type=str, default="/root/autodl-tmp/HealthGPT/VQA-RAD/question.json")
    parser.add_argument("--output_file", type=str, default="/root/autodl-tmp/HealthGPT/VQA-RAD/question.csv")
    ...
    for line in tqdm(questions):
        ...
        Q = line["question"]
        img_path = os.path.join("/root/autodl-tmp/HealthGPT/VQA-RAD/VQA_RAD Image Folder", line["image"])
        A = line["answer"]
        
        #############################################################
        #step1：用 DeepSeek R1 提取图像解读的重点信息
        #prompt1
        prompt_step1 = (
        ...
        prompt_step2 = (
            ...
            f"Key features to observe: {response_step1_text}")
        question = prompt_step2
        
        ##################################################################################
        #step2:用HealthGPT生成详细描述
        ...
        prompt_step3 = (
            ...
            f"Image description: {response_step2}\n"
            f"Question: {Q}\n"
        ...
        result = pd.DataFrame([[line["image"], Q,A,response_step1_text,response_step2,output_text]], columns=["Image_Path","Q","A", "step1","step2","step3"])
        
        # 追加写入 CSV，避免占用过多内存
        result.to_csv(output_file, mode='a', header=False, index=False, encoding="utf-8-sig")
