...
def infer():
    ...
    parser.add_argument('--do_sample', type=bool, default=True) #do_sample=True才允许对temperature进行调试
    parser.add_argument('--temperature', type=float, default=0.0)
    parser.add_argument('--top_p', type=float, default=None)
    parser.add_argument('--num_beams', type=int, default=1)
    parser.add_argument('--max_new_tokens', type=int, default=1024)
    parser.add_argument("--question-file", type=str, default="/root/autodl-tmp/HealthGPT/question_VQARAD3.jsonl")
    parser.add_argument("--output_file", type=str, default="/root/autodl-tmp/HealthGPT/response_output_com_batchinfer-VQARAD3.csv")
    ...
    for line in tqdm(questions):
        ...
        question = line["text"]
        img_path = os.path.join("/root/autodl-tmp/HealthGPT/VQA-RAD-xray", line["image"])
        ...
        result = pd.DataFrame([[line["image"],args.temperature,response]], columns=["Image_Path", "temperature","Response"])
        # 追加写入 CSV，避免占用过多内存
