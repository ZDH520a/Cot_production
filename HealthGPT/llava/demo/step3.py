...
class Step3PostProcessor:
    ...
    def step3_single_request(self, image_description: str, question: str, answer: str) -> Dict[str, Any]:
        """第三步：单个推理请求"""
        system_message = """You are a professional medical imaging physician specializing in image-based diagnosis. 
            When interpreting each case:
            1. Examine the image itself—do not refer to or quote any external description.
            2. Build your reasoning step by step as if you don't yet know the answer.
            3. Avoid meta-statements like "as stated in the image description" or "according to the ground truth."
            4. When your analysis is complete, OUTPUT ONLY THE FINAL ANSWER, matching exactly the expected answer provided.
            """
...
"""},
    ...
    def postprocessing_step1(self, reasoning_content: str) -> str:
        ...
        system_prompt = (
            ...
            "Do not use phrases like 'the description says', 'it is stated', or 'it is described'.\n"
    ...
    def postprocessing_step2(self, text: str) -> str:
        ...
        logic_prompt = (
            ...
            "6. Very importantly, retain natural reasoning transitions such as "
            "'Okay, let's start by...', 'First, I observe...', 'Next...', etc., "
    ...
    def fanyi(self, text: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model="deepseek-chat",
                messages=[
                    {"role": "system", "content": "You are a translation assistant, please translate the English I input into Chinese, keeping the special symbols inside.Do not output anything other than the translation."},
                    {"role": "user", "content": text},
                ],  
                stream=False
            )
    ...
    def batch_process(self, input_data: List[Dict], max_workers: int = 5, 
        ...
        """批量并行处理"""
        print(f"开始第三步和后处理，共 {len(input_data)} 条数据")
        ...
        for batch_start in range(0, len(input_data), batch_size):
            ...
            print(f"批次完成: 成功 {successful}/{len(batch_data)}, 需要后处理 {needs_postprocessing}")
    ...
    if output_file.endswith('.json'):
        ...
        for result in results:
            row = {
                ...
                "是否需要后处理": str(result.get("needs_postprocessing", False)),
                "后处理思维链": result.get("postprocessed_output", ""),
                "后处理思维链翻译": result.get("postprocessed_translation", ""),
...
def main():
    parser = argparse.ArgumentParser(description="第三步推理和后处理")
    parser.add_argument("--step2_file", type=str,
                       default="/root/autodl-tmp/HealthGPT/ZDH/train_0217/step2_results.json",
                       help="Step2结果文件路径")
    parser.add_argument("--output_file", type=str,
                       default="/root/autodl-tmp/HealthGPT/ZDH/train_0217/step3_results_new.csv",
                       help="输出文件路径")
    parser.add_argument("--api_key", type=str, 
                       default="sk-3d990190200d42f2b500fe6d8117aa76",
                       help="DeepSeek API密钥")
    ...
    try:
        ...
        print(f"需要后处理: {needs_postprocessing}")
